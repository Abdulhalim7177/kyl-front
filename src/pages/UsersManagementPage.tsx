...
const SIDEBAR_ITEMS = [
  ...
  { name: 'Political Parties', icon: Flag, active: false },
  { name: 'Elections', icon: Vote, active: false },
  { name: 'Offices', icon: Building2, active: false },
  { name: 'Districts', icon: MapPin, active: false },
  { name: 'Candidates', icon: UserCheck, active: false },
  { name: 'Elected Officials', icon: UserCheck, active: false },
  { name: 'Blogs', icon: FileText, active: false },
  { name: 'Activity Logs', icon: Activity, active: false },
]
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-white">
      <div className="mb-6 pt-6 flex justify-center">
        <img src="/logo.png" alt="KYL Logo" className="h-[4.5rem] w-auto object-contain cursor-pointer" onClick={() => navigate('/')} />
      </div>
        ...
        {SIDEBAR_ITEMS.map((item, index) => {
          ...
          return (
            <a 
              ...
              className={`flex items-center px-4 py-3 rounded-xl font-medium transition-colors ${item.active ? 'bg-[#dcfce7]/50 text-[#146c4f]' : 'text-gray-600 hover:bg-gray-50'}`}
            >
              <IconComponent className={`w-5 h-5 mr-3 ${item.active ? 'text-[#146c4f]' : 'text-gray-400'}`} />
              {item.name}
            </a>
          )
        })}
      ...
      {/* Bottom Profile and Logout via dropdown / popup */}
  ...
  return (
    <div className="flex h-[100dvh] bg-[#f8fafc] font-sans text-sm overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-full relative overflow-hidden">
        ...
        <header className="h-[72px] bg-white md:bg-transparent border-b border-border md:border-none flex items-center justify-between px-4 sm:px-8 z-10 shrink-0">
          <div className="flex items-center gap-3">
             ...
             <h1 className="text-lg sm:text-xl font-semibold text-gray-800">Admin / User Management</h1>
          </div>
        ...
        {/* Users Content Block */}
        <main className="flex-1 overflow-y-auto overflow-x-hidden w-full px-4 sm:px-8 pb-8 space-y-6 max-w-[1400px] mx-auto z-10 relative">
          {/* Header Text & Actions */}
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mt-2 sm:mt-4">
             <div>
                <h2 className="text-2xl sm:text-3xl text-gray-900 font-serif font-medium tracking-tight">Users.</h2>
                <p className="text-sm text-gray-500 mt-1">View users, assign permissions, and manage access levels in one place.</p>
             </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-x-auto relative">
             ...
             <div className="bg-gray-50/50 p-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between text-sm gap-4">
                <span className="text-gray-500 font-medium ml-2">Showing 1 to 10 of 1,248 users</span>
                <div className="flex items-center gap-1 overflow-x-auto pb-1 sm:pb-0 w-full sm:w-auto justify-center">
                   ...
                   <button className="w-8 h-8 shrink-0 flex items-center justify-center rounded hover:bg-white text-gray-700 font-medium">2</button>
                   <button className="w-8 h-8 shrink-0 flex items-center justify-center rounded hover:bg-white text-gray-700 font-medium">3</button>
                   <span className="w-8 h-8 shrink-0 flex items-center justify-center text-gray-400">...</span>
                   <button className="w-8 h-8 shrink-0 flex items-center justify-center rounded hover:bg-white text-gray-700 font-medium">125</button>
      ...
      {/* --- Reusable Animated Modal Dialogs --- */}
      ...
      >
        <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
           ...
           <div className="mt-4 pt-2">
             <label className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2 block">Users to be removed</label>
             <div className="flex items-center -space-x-2">
               {Array.from({length: Math.min(3, numSelected)}).map((_, i) => (
                 <Avatar key={i} className="w-8 h-8 border-2 border-white bg-gray-200">
                    <AvatarImage src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${i+10}`} />
                    <AvatarFallback>U{i}</AvatarFallback>
                 </Avatar>
               ))}
               {numSelected > 3 && (
                 <div className="w-8 h-8 rounded-full border-2 border-white bg-blue-100 text-blue-700 flex items-center justify-center text-xs font-bold z-10">
                   +{numSelected - 3}
                 </div>
               )}
             </div>
      ...
      >
        <div className="space-y-4 pr-1">
           ...
           <div className="bg-gray-50 rounded-lg p-3 sm:p-4 text-gray-600 text-sm border border-gray-100">
             This will immediately revoke access for the selected users. They will no longer be able to log in or access organization resources. You can reactivate them later from the Inactive tab.
           </div>
           ...
           <div className="space-y-4 mt-4 sm:mt-6">
             <div className="space-y-1.5">
                ...
                <Select>
                  <SelectTrigger><SelectValue placeholder="Select a primary reason" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="security">Security Protocol</SelectItem>
                    <SelectItem value="leave">Temporary Leave</SelectItem>
                    <SelectItem value="other">Other Administraton</SelectItem>
                  </SelectContent>
                </Select>
             </div>
             ...
             <div className="space-y-1.5">
                ...
                <textarea 
                  className="w-full border border-gray-200 rounded-md p-3 text-sm min-h-[80px] focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500" 
                  placeholder="Provide any context for this action..." 
                />
             </div>
           </div>
        </div>
        ...
        <div className="space-y-4">
           <div className="bg-orange-50/50 rounded-xl p-3 sm:p-4 border border-orange-100 flex gap-3">
              ...
              <div>
                 ...
                 <p className="text-gray-500 text-sm mt-1">This will change the permissions and access levels for {numSelected} users.</p>
              </div>
           </div>
           ...
           <div className="space-y-2 mt-4 pt-2">
             ...
             <Select defaultValue="party">
               ...
               <SelectContent>
                 ...
                 <SelectItem value="support">Party Support Admin</SelectItem>
               </SelectContent>
             </Select>
           </div>
        </div>
      ...
      {/* 4. Logout Dialog (Demonstrating default variant) */}
      <AnimatedConfirmDialog
        open={isLogoutOpen}
